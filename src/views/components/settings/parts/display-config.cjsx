  ...
  render: ->
    <Grid>
      ...
      <Col xs=4>
        <Input type="select"
         ...
         disabled={!@state.useFixedResolution} >
          <option key={-1} value={@state.gameWidth} hidden>{Math.round(@state.gameWidth/W*100)}%</option>
          {
            i = 0
            while i < 4
              i++
              <option key={i} value={i * W/2}>
                {i * 50}%
              </option>
          }
        </Input>
      ...
      <Col id="poi-resolution-config" xs=12 style={display: 'flex', alignItems: 'center'}>
        ...
        <div style={flex: 1}>
          <Input type="number" value={Math.round(@state.gameWidth * H / W)} readOnly />
        </div>
        <div style={flex: 'none', width: 15, paddingLeft: 5}>
          px
        </div>
