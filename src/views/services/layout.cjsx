...
adjustSize = ->
  ...
  [].forEach.call $$('poi-app div.poi-app-tabpane'), (e) ->
    if window.layout == 'horizontal'
      ...
      e.style.height = "#{(window.innerHeight - Math.ceil(H * factor) - poiControlHeight) / window.zoomLevel - poiControlHeight}px"
  ...
  if window.layout == 'horizontal'
    factor = Math.ceil((window.innerHeight - poiControlHeight) / H * 100) / 100.0
    if window.innerWidth - factor * W < cap
      factor = Math.ceil((window.innerWidth - cap) / W * 100) / 100.0
  else
    factor = Math.ceil(window.innerWidth / W * 100) / 100.0
    if window.innerHeight - factor * H < cap
      factor = Math.ceil((window.innerHeight - cap) / H * 100) / 100.0
  if window.webviewWidth > 0.00001
    factor = Math.ceil(window.webviewWidth / W * 100) / 100.0
  ...
  if url != 'http://www.dmm.com/netgame/social/-/gadgets/=/app_id=825012/' and !(url?.startsWith('http://osapi.dmm.com/gadgets/ifr'))
  ...
  if window.layout == 'horizontal'
    adjustWebviewHeight "#{Math.min(Math.floor(H * factor), window.innerHeight - poiControlHeight)}px"
    $('kan-game #webview-wrapper')?.style?.width = "#{Math.floor(W * factor)}px"
    $('kan-game #webview-wrapper')?.style?.marginLeft = '0'
    $('kan-game')?.style?.marginTop = "#{Math.max(0, Math.floor((window.innerHeight - H * factor - poiControlHeight) / 2.0))}px"
  else
    adjustWebviewHeight "#{Math.floor(H * factor)}px"
    $('kan-game #webview-wrapper')?.style?.width = "#{Math.floor(W * factor)}px"
    $('kan-game #webview-wrapper')?.style?.marginLeft = "#{Math.max(0, Math.floor((window.innerWidth - Math.floor(W * factor)) / 2.0))}px"
...
handleResize = ->
  if window.layout == 'horizontal'
    ...
    if webviewWidth != -1
      ...
      $('kan-game').style.flex = Math.floor(window.webviewFactor * W)
      $('poi-app').style.flex = window.innerWidth - Math.floor(window.webviewFactor * W)
  if !window._delay
    adjustSize()
  else
    window._delay = false
...
handleTitleSet = ->
  try
    ...
    url = null
  return if url != 'http://www.dmm.com/netgame/social/-/gadgets/=/app_id=825012/' and !(url?.startsWith('http://osapi.dmm.com/gadgets/ifr'))
  @insertCSS """
    #ntg-recommend {
...
changeBounds = ->
  ...
  if layout == 'horizontal'
    # Previous vertical
    newHeight = window.innerWidth / W * H + 30
    newWidth = window.innerWidth / 5 * 7
  else
    # Previous horizontal
    newHeight = window.innerWidth / 7 * 5 / W * H + 420
